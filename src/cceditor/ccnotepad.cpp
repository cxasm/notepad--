...
static bool openSwapFileForWrite(QFile& file, const QString& filePath)
{
#ifdef Q_OS_WIN
	file.setFileName(filePath);
	return file.open(QIODevice::ReadWrite | QIODevice::Truncate);
#else
	QFileInfo fi(filePath);
	...
	int fd = ::open(nativePath.constData(), O_RDWR | O_CREAT | O_TRUNC | O_CLOEXEC | O_NOFOLLOW, 0600);
	if (fd < 0)
	{
		return false;
	}
...
bool  CCNotePad::saveFile(QString fileName, ScintillaEditView* pEdit, bool isBakWrite, bool isStatic, bool isClearSwpFile)
{
	...
	if (isSwapFile)
	{
		opened = openSwapFileForWrite(file, file.fileName());
	}
	else
	{
		opened = file.open(QIODevice::ReadWrite | QIODevice::Truncate);
	}
